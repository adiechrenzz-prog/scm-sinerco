...
const DashboardKemala = () => {
  ...
  const handleLogout = async () => {
    try {
      // 1. Hapus cache agar login berikutnya bersih
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-header">
        <h2 className="field-title">FIELD GUNUNG KEMALA</h2>
      </div>
      ...
      <div className="svg-wrapper">
        <svg viewBox="0 0 1000 1000" className="main-svg">
          ...
          {/* Menu Nodes */}
          {menus.map((m, i) => {
            const angle = (i * (2 * Math.PI) / menus.length) - Math.PI / 2;
            ...
            return (
              <g key={i} className="menu-node" onClick={() => navigate(m.path)}>
                ...
                <text x={x} y={y} textAnchor="middle" className="node-label">
                  {words.map((word, idx) => {
                    // Kalkulasi offset vertikal agar teks presisi di tengah lingkaran
...
export default DashboardKemala;