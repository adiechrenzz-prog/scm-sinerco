...
export default function DashboardLimau() {
  ...
  const handleLogout = async () => {
    try {
      // 1. Bersihkan cache data field untuk keamanan navigasi
      localStorage.removeItem("user_field_data");
      
      // 2. Sign out Firebase
      await signOut(auth);
      
      // 3. Pindah ke login dengan replace history
      navigate("/login", { replace: true });
    } catch (error) {
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-header">
        <h2 className="field-title">FIELD LIMAU</h2>
      </div>
            ...
            return (
              <g key={i} className="menu-node" onClick={() => navigate(m.path)}>
                ...
                <text x={x} y={y} textAnchor="middle" className="node-label">
                  {words.map((word, idx) => {
                    // Kalkulasi posisi vertikal teks agar simetris di tengah
