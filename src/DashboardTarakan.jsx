...
export default function DashboardTarakan() {
  ...
  const handleLogout = async () => {
    try {
      // 1. Bersihkan cache data field untuk keamanan data antar akun
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-header">
        <h2 className="field-title">FIELD TARAKAN</h2>
      </div>
            ...
            return (
              <g key={i} className="menu-node" onClick={() => navigate(m.path)}>
                ...
                <text x={x} y={y} textAnchor="middle" className="node-label">
                  {words.map((word, idx) => {
                    // Kalkulasi posisi vertikal teks agar simetris di tengah lingkaran (1, 2, atau 3 kata)
