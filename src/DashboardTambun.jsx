...
const DashboardTambun = () => {
  ...
  const handleLogout = async () => {
    try {
      // 1. Hapus cache data field agar tidak terjadi throttling saat login akun berbeda
  ...
  return (
    <div className="dashboard-container">
      <div className="dashboard-header">
        <h2 className="field-title">FIELD TAMBUN</h2>
      </div>
            ...
            return (
              <g key={i} className="menu-node" onClick={() => navigate(m.path)}>
                ...
                <text x={x} y={y} textAnchor="middle" className="node-label">
                  {words.map((word, idx) => {
                    // Kalkulasi posisi vertikal agar teks (1-3 baris) tetap di tengah lingkaran
...
export default DashboardTambun;